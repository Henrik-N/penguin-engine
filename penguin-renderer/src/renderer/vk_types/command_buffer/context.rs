...
impl VkContext {
    pub fn reset_command_buffer(&self, command_buffer: vk::CommandBuffer, reset_flags: vk::CommandBufferResetFlags) {
        unsafe {
            self.device.reset_command_buffer(command_buffer, reset_flags) //  vk::CommandBufferResetFlags::RELEASE_RESOURCES
                .expect("Failed to reset command buffer");
        }
    }

    pub fn reset_command_pool(&self, command_pool: vk::CommandPool, reset_flags: vk::CommandPoolResetFlags) {
        unsafe {
            self.device.reset_command_pool(command_pool, reset_flags)
...
mod alloc {
    ...
    impl VkContext {
        pub fn alloc_command_pool(&self, queue_family_index: u32, pool_flags: vk::CommandPoolCreateFlags)
                                  -> vk::CommandPool {
            ...
            log::trace!("Creating command pool");
            unsafe { self.device.create_command_pool(&command_pool_create_info, None) }
                .expect("Command pool couldn't be created.")
        }

        pub fn allocate_command_buffers(&self, command_pool: vk::CommandPool, count: u32) -> Vec<vk::CommandBuffer> {
            ...
            unsafe {
                self.device.allocate_command_buffers(&create_info)
            }.expect("couldn't allocate command buffers")
        }

        pub fn alloc_command_pool_with_buffers(&self, buffer_count: u32,
