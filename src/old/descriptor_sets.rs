...
mod descriptor_set_builder {
    ...
    use crate::renderer::memory::MemType;
    ...
    impl<'a> DescriptorSetBuilder<'a> {
        ...
        pub fn descriptor_set<DescriptorDataType: Sized>(
            ...
            // it's buffer region
            let packed_align = memory::packed_range_from_min_align::<DescriptorDataType>(min_ubuffer_offset_align);
        ...
        pub fn build(self) {
            ...
            &self
                .descriptors
                .iter()
                .any(|desc| desc.mem_type == MemType::Dynamic);
            ...
            let persistent_bytes_count = {
                ...
                for desc in self
                    .descriptors
                    .iter()
                    .filter(|desc| desc.mem_type == MemType::Persistent)
                {
