...
pub struct PAllocatedBuffer {
    pub buffer: vk::Buffer, // handle to gpu-side buffer
    buffer_memory: vk::DeviceMemory,
}
impl PAllocatedBuffer {
...
pub fn create_vertex_buffer(
    ...
    let allocated_memory =
        unsafe { device.allocate_memory(&allocate_info, None) }.expect("Couldn't allocate memory");
    ...
    // map memory
    let ptr_to_memory = unsafe { device.map_memory(
            allocated_memory,
            offset,
    ...
    let mut memory_slice: Align<Vertex> = unsafe { Align::new(
        ptr_to_memory,
    ...
    unsafe {
        ...
        device.bind_buffer_memory(buffer, allocated_memory, 0)
