...
pub(crate) mod init {
    ...
    pub fn create_command_pool_and_buffer(device: &ash::Device, graphics_queue_index: u32) -> (vk::CommandPool, vk::CommandBuffer) {
...
pub fn record_submit_command_buffer<RenderPassFn: FnOnce(&ash::Device, vk::CommandBuffer, vk::Framebuffer)>(
    device: &ash::Device,
    ...
    unsafe {
        ...
        device.wait_for_fences(&[fence], true, u64::MAX).expect("Couldn't wait for fences. Timed out?");
        log::trace!("Resetting fence.");
        device.reset_fences(&[fence]).expect("Couldn't reset fences.");
    }

        // reset command buffer
    unsafe {
        device.reset_command_buffer(command_buffer, vk::CommandBufferResetFlags::empty()) //  vk::CommandBufferResetFlags::RELEASE_RESOURCES)
    ...
    unsafe {
        ...
        device.begin_command_buffer(command_buffer, &cmd_buffer_begin_info)
    ...
    unsafe {
        ...
        device.queue_submit(submit_queue, &[submit_info.build()], fence);
    }
}
